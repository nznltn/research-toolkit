%[text] This function is designed to create a pattern for rectangular spotted subsampling inside a hexagonal shaped region, potentially aiming a circular object or sample. Hexagonals are useful as approximations to simulate circular regions when lines and edges are required as the boundaries. It is aimed to create constant distances between the subsamples and do a represenatative and symmetrical sampling to distribute the subsample centers uniformly.
%[text] The function inputs, "rad" is the radius of the circular space considered; "subsampleRatio" is the ratio (0-1) of the object or sample on how much of the region is desired to cover or scan; "r\_h" and "r\_w" are the height and width of the rectangles.    
%[text] The function outputs "subsamples" is the coordinates of the center of the rectangular subsamples; "hex\_x" and "hex\_y" are the coordinates of the hexagonals. 
%[text] A hexagonal's smallest edge to edge distance is used for fitting the small side of the rectangles;  the hexagonal's largest corner to corner distance is used for fitting the large side of the rectangles. After the regions is filled at the max level, selection of the subsamples is made by kmeans to achieve a uniform placement of subsample centers.  
%[text] 
function [subsamples, hex_x, hex_y] = HexagonalSampling(rad, subsampleRatio, r_h, r_w)

% shortest distance between two parallel sides
...
% Plotting the result
figure;
scatter(grid_x, grid_y, 'b', 'filled');
axis equal;
xlabel('X'); ylabel('Y');
...
scatter(inside_x, inside_y, 'b', 'filled');  % Grid points inside hexagon
axis equal;
xlabel('X'); ylabel('Y');
