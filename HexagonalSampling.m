%[text] This function is designed to create a pattern for rectangular spotted subsampling inside a hexagonal shaped region, potentially aiming a circular object or sample. It is aimed to create constant distances between the subsamples and do a represenatative and symmetrical sampling to distribute the subsample centers uniformly.
%[text] 
%[text] Hexagonals are useful as approximations to simulate circular regions when lines and edges are required as the boundaries. A hexagonal's smallest edge to edge distance is used for fitting the small side of the rectangles;  the hexagonal's largest corner to corner distance is used for fitting the large side of the rectangles. After the regions is filled at the max level, selection of the subsamples is made by kmeans to achieve a uniform placement of subsample centers.  
%[text] 
function [subsamples, hex_x, hex_y] = HexagonalSampling(rad, subsampleRatio, r_h, r_w)

% Generates rectangular subsample points inside a hexagonal area and plots them
%
% Inputs:
